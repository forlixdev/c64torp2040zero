...
# Crea l'oggetto per il tasto RESTORE (non fa parte della matrice).
# Collega il filo Nero (Pin 3) a GP26 e il filo Bianco (Pin 1) a GND.
restore_button = keypad.Keys((board.GP26,), value_when_pressed=False, pull=True)
RESTORE_KEYCODE = Keycode.PAGE_UP # Perfetto per VICE. Alternative valide: PAUSE, PRINT_SCREEN o DELETE
...
while True:
    ...
    # Controlla se c'è un evento per il tasto RESTORE
    restore_event = restore_button.events.get()
    
    if restore_event:
        if restore_event.pressed:
            kbd.press(RESTORE_KEYCODE)
            # print(f"Premuto: RESTORE -> Codice USB {RESTORE_KEYCODE}")
        elif restore_event.released:
            kbd.release(RESTORE_KEYCODE)
